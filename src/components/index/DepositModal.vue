    ...
    <Modal :close="cancel" :visible="visible && step === 1">
      ...
      <template v-slot:body>
        <div class="pb-15">
          ...
          <button v-if="connectStatus === 'onboarding'" class="w-full mb-16 button button--success" disabled>Waiting for
            MetaMask...
          </button>
          <button v-else-if="connectStatus === 'connecting'" class="w-full mb-16 button button--success" disabled>
            Connecting to MetaMask...
          </button>
          <button v-else-if="hasMetaMask" class="w-full mb-16 button button--success" @click="connect">Connect
            MetaMask
          </button>
          <button v-else class="w-full mb-16 button button--success" @click="installMetaMask"
                  :disabled="!browserSupport">Click to install MetaMask
          </button>
        ...
        <div class="flex justify-between">
          <div>
            ...
            <span class="sub-heading d-block text-gray text-caption">
              <Amount :value="xctBalance" currency="EDGE"/> available
            </span>
          </div>
          <div>
    ...
    <Modal :close="cancel" :visible="visible && step === 3">
      ...
      <template v-slot:footer>
        <div class="px-24 pt-40 pb-40 border-t border-gray-700 border-opacity-30">
          <button class="block w-full mx-auto text-center button button--success md:w-3/6" @click="cancel">Close
          </button>
        </div>
      </template>
    </Modal>
...
export default {
  ...
  validations() {
    return {
      amount: [
        ...
        helpers.withParams(
          {b: this.xctBalance, p: this.amountParsed},
          helpers.withMessage('Insufficient funds.', () => {
            if (isNaN(this.amountParsed)) return false
            return this.amountParsed <= this.xctBalance
          })
        )
      ]
    }
  ...
  methods: {
    ...
    async setMaxAmount() {
      this.amount = (Math.floor(this.xctBalance * 1e6) / 1e6).toString()
